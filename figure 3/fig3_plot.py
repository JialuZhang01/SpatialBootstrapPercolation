...
for i0 in range(3):  
    for j0 in range(3): 
        ...
        if i0==0: # 第一行是P∞
            ...
            c0=0
            f1 = open('deal_result/3、12、1000_cascade_dynamic/'+'T='+str(T_active) +'_avgk='+str(avg_k) +'_t_S(gc)_zeta'+str(zetas[j0])+'.txt','r', encoding='utf-8-sig')
            for line_f1 in f1:
                x_average = []
                y_average = []
...
plt.tight_layout()
plt.savefig('deal_result/FIG3/fig3.pdf')
plt.show()
